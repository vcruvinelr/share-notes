from fastapi import Depends, HTTPException, status, WebSocket, Request, Response
...
async def get_current_user(
    ...
    if credentials:
        ...
        try:
            ...
            print(f"DEBUG: JWT decoded - keycloak_id={keycloak_id}, email={email}, username={username}")
            ...
            if keycloak_id is None:
                ...
                if not user:
                    ...
                    try:
                        ...
                        if email:
                            result = await db.execute(select(User).where(User.email == email, User.is_anonymous == False))
                            user = result.scalar_one_or_none()
                        if user:
