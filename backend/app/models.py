...
class User(Base):
    ...
    permissions = relationship("NotePermission", back_populates="user", cascade="all, delete-orphan")
...
class Note(Base):
    ...
    note_type = Column(SQLEnum(NoteType, values_callable=lambda x: [e.value for e in x]), nullable=False, default=NoteType.STANDARD)
    ...
    permissions = relationship("NotePermission", back_populates="note", cascade="all, delete-orphan")
