...
from app.models import User, Subscription, SubscriptionStatus, Note
...
async def create_checkout_session(
    ...
    if current_user.is_anonymous:
        raise HTTPException(status_code=403, detail="Anonymous users cannot subscribe. Please create an account.")
...
async def stripe_webhook(
    ...
    if event["type"] == "checkout.session.completed":
        ...
        result = await db.execute(
            select(Subscription).where(Subscription.stripe_subscription_id == stripe_subscription.id)
        )
        subscription = result.scalar_one_or_none()
