...
from app.schemas import WSMessage, WSEditMessage, WSCursorMessage
import logging
...
class ConnectionManager:
    ...
    async def broadcast_to_note(self, note_id: str, message: dict, exclude_websocket: Optional[WebSocket] = None):
    ...
    async for db in get_db():
        ...
        if token and (not user_id or user_id == ""):
            ...
            try:
                ...
                # Get user from token
                authenticated_user = await get_current_user(mock_request, mock_response, credentials, db)
